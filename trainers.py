"""
Script having code for pre-training and fine-tuning
the model used
...
class Trainer:
    ...
    def dss_cross_entropy(self, seq_out, pos_ids, neg_ids):
        ...
        """
        # [batch seq_len hidden_size]
        pos_emb = self.model.item_embeddings(pos_ids)  # [B L H]
        neg_emb = self.model.item_embeddings(neg_ids)  # [B L H]

        seq_out_t = seq_out.transpose(1, 2)  # [B H K]

        pos_logits = torch.matmul(pos_emb, seq_out_t).max(dim=-1)[0].reshape(-1)  # [B*L]
        neg_logits = torch.matmul(neg_emb, seq_out_t).max(dim=-1)[0].reshape(-1)  # [B*L]

        istarget = (pos_ids > 0).view(pos_ids.size(0) * self.model.args.max_seq_length).float()  # [B*S]
        loss = torch.sum(- torch.log(torch.sigmoid(pos_logits) + 1e-24) * istarget -
                         torch.log(1 - torch.sigmoid(neg_logits) + 1e-24) * istarget) / torch.sum(istarget)
